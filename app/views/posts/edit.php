    ...
    <div class="col-xs-12 offset-sm-3 col-sm-6 offset-md-4 col-md-4">
      ...
      <form action="<?= URLROOT; ?>/posts/edit/<?= $data['id']; ?>" method="post">
        ...
        <div class="form-floating mb-2">
          <input type="text" name="title" class="form-control <?= (!empty($data['title_err'])) ? 'is-invalid' : '' ?>" id="floatingTitle" placeholder="Title" value="<?= $data['title']; ?>">
          <span class="invalid-feedback"><?= $data['title_err']; ?></span>
          <label for="floatingTitle">Title</label>
        </div>

        <div class="form-floating mb-2">
          <textarea name="body" class="form-control <?= (!empty($data['body_err'])) ? 'is-invalid' : '' ?>" placeholder="Body content goes here..." id="floatingBody" style="height: 200px;"><?= $data['body']; ?></textarea>
